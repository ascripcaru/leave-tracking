...
function handleEmploymentAnniversary(params) {
    try {
        ...
        sendMail(toEmail, emailSubject, 'employmentAnniversary', params);
    } catch (error) {
        logger.error(error);
    }
}
...
function handleNewUser(params) {
    try {
        ...
        sendMail(email, emailSubject, 'newUser', params);
    } catch (error) {
        logger.error(error);
    }
}

function handlePasswordReset(params) {
    try {
        ...
        sendMail(email, emailSubject, 'resetPassword', params);
    } catch (error) {
        logger.error(error);
    }
}
